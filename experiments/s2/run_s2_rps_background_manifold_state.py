...
def decide(rows: List[dict]) -> None:
    ...
    for row in rows:
        if row is control:
            ...
            continue
        recovered = row['bonn_rear_points_sum'] >= 100.0
        bg_gain = row['bonn_rear_true_background_sum'] > control['bonn_rear_true_background_sum']
        ghost_ok = row['bonn_rear_ghost_sum'] <= 15.0
        metric_ok = row['bonn_ghost_reduction_vs_tsdf'] >= control['bonn_ghost_reduction_vs_tsdf']
        row['decision'] = 'iterate' if recovered and bg_gain and ghost_ok and metric_ok else 'abandon'
...
def write_distribution(payloads: Dict[str, dict], path_md: Path) -> None:
    ...
    lines += ['', '检查重点：rear 总量是否恢复到 `>=100`，以及 `true_background` 是否提升且 `ghost <= 15`。']
    path_md.write_text('\n'.join(lines) + '\n', encoding='utf-8')
...
def write_analysis(rows: List[dict], path_md: Path) -> None:
    ...
    lines = [
        ...
        '',
        '## 1. 是否解决了“零点”问题',
        f"- 控制组 `38`: rear_points=`{control['bonn_rear_points_sum']:.0f}`",
        f"- 本轮最佳候选 `{best['variant']}`: rear_points=`{best['bonn_rear_points_sum']:.0f}`",
    ]
    if best['bonn_rear_points_sum'] >= 100.0:
        lines.append('- 结论：稳定背景流形状态至少避免了 rear points 归零。')
    else:
        lines.append('- 结论：稳定背景流形状态仍未能恢复足够的 rear points。')
    lines += [
        '',
        ...
        f"- 最佳候选 `{best['variant']}`: TB=`{best['bonn_rear_true_background_sum']:.0f}`, Ghost=`{best['bonn_rear_ghost_sum']:.0f}`, Hole/Noise=`{best['bonn_rear_hole_or_noise_sum']:.0f}`",
    ]
    if best['bonn_rear_true_background_sum'] > control['bonn_rear_true_background_sum']:
        lines.append('- True Background 点数有所提升。')
    else:
        lines.append('- True Background 点数没有提升。')
    if best['bonn_rear_ghost_sum'] <= 15.0:
        lines.append('- Ghost 点数仍控制在可接受范围内。')
    else:
        lines.append('- Ghost 点数仍然超出本轮目标。')
    lines += [
        '',
        '## 3. 指标是否提升',
        f"- 控制组 `38`: Bonn `ghost_reduction_vs_tsdf = {control['bonn_ghost_reduction_vs_tsdf']:.2f}%`, `Comp-R = {control['bonn_comp_r_5cm']:.2f}%`",
        f"- 最佳候选 `{best['variant']}`: Bonn `ghost_reduction_vs_tsdf = {best['bonn_ghost_reduction_vs_tsdf']:.2f}%`, `Comp-R = {best['bonn_comp_r_5cm']:.2f}%`",
    ]
...
def main() -> None:
    ap = argparse.ArgumentParser(description='S2 background manifold state runner.')
    ...
    variants = [
        dict(base_spec, name='38_bonn_state_protect', output_name='38_bonn_state_protect_bg_manifold_control'),
        dict(
            base_spec,
            name='48_stable_background_memory_state',
            ...
            bonn_rps_bg_manifold_weight_gain=0.70,
            bonn_rps_bg_manifold_rho_ref=0.07,
            bonn_rps_bg_manifold_weight_ref=0.28,
            bonn_rps_bg_manifold_history_weight=0.42,
            bonn_rps_bg_manifold_obstruction_weight=0.12,
            bonn_rps_bg_manifold_visible_lo=0.18,
            bonn_rps_bg_manifold_visible_hi=0.42,
        ),
        dict(
            base_spec,
            name='49_relaxed_manifold_guided_generation',
            bonn_rps_bg_manifold_state_enable=True,
            bonn_rps_bg_manifold_alpha_up=0.10,
            bonn_rps_bg_manifold_alpha_down=0.01,
            bonn_rps_bg_manifold_rho_alpha=0.12,
            ...
            bonn_rps_bg_manifold_visible_lo=0.16,
            bonn_rps_bg_manifold_visible_hi=0.40,
            bonn_rps_history_manifold_enable=True,
            bonn_rps_history_manifold_visible_min=0.32,
            bonn_rps_history_manifold_obstruction_min=0.20,
            bonn_rps_history_manifold_bg_weight=0.80,
            bonn_rps_history_manifold_geo_weight=0.15,
            bonn_rps_history_manifold_static_weight=0.05,
            bonn_rps_history_manifold_blend=0.92,
    ...
    compare_csv = out_dir / 'S2_RPS_BACKGROUND_MANIFOLD_STATE_COMPARE.csv'
    compare_md = out_dir / 'S2_RPS_BACKGROUND_MANIFOLD_STATE_COMPARE.md'
    dist_md = out_dir / 'S2_RPS_BACKGROUND_MANIFOLD_STATE_DISTRIBUTION.md'
    analysis_md = out_dir / 'S2_RPS_BACKGROUND_MANIFOLD_STATE_ANALYSIS.md'
