...
def decide(rows: List[dict]) -> None:
    ...
    for row in rows:
        ...
        bg_gain = row['bonn_rear_true_background_sum'] >= 8.0
...
def write_distribution(payloads: Dict[str, dict], path_md: Path) -> None:
    ...
    lines += ['', '重点检查：rear 是否恢复到 `>=80`，TB 是否达到 `>=8`，Ghost 是否仍 `<=10`。']
    path_md.write_text('\n'.join(lines) + '\n', encoding='utf-8')
...
def write_analysis(rows: List[dict], path_md: Path) -> None:
    ...
    if best['bonn_rear_points_sum'] >= 80.0:
        lines.append('- rear 覆盖量已恢复到目标区间。')
    else:
        lines.append('- rear 覆盖量仍未恢复到目标区间。')
    if best['bonn_rear_true_background_sum'] >= 8.0:
        lines.append('- True Background 达到了本轮目标。')
    else:
        lines.append('- True Background 仍未达到本轮目标。')
    if best['bonn_rear_ghost_sum'] <= 10.0:
        lines.append('- Ghost 仍保持在低位。')
    else:
        lines.append('- Ghost 没有被控制住。')
    lines += [
        '',
        '## 2. 指标是否改善',
...
def main() -> None:
    ap = argparse.ArgumentParser(description='S2 high coverage bridge runner.')
    ...
    variants = [
        dict(base_spec, name='38_bonn_state_protect', output_name='38_bonn_state_protect_high_coverage_bridge_control'),
        dict(
            base_spec,
            name='59_relaxed_occlusion_tunneling',
        ...
        dict(
            ...
            bonn_rps_bg_bridge_min_step=2,
            bonn_rps_bg_bridge_max_step=4,
            bonn_rps_bg_bridge_gain=0.80,
            bonn_rps_bg_bridge_relaxed_dyn_max=0.50,
            bonn_rps_bg_bridge_keep_multi_hits=True,
            bonn_rps_bg_bridge_max_hits_per_source=4,
            bonn_rps_bg_bridge_cone_enable=True,
            bonn_rps_bg_bridge_cone_radius_cells=1,
            bonn_rps_bg_bridge_cone_gain_scale=0.50,
            bonn_rps_rear_hybrid_filter_enable=True,
            bonn_rps_rear_hybrid_bridge_support_min=0.18,
            bonn_rps_rear_hybrid_dyn_max=0.26,
            bonn_rps_rear_hybrid_manifold_min=0.18,
        ),
    ]
    ...
    compare_csv = out_dir / 'S2_RPS_HIGH_COVERAGE_BRIDGE_COMPARE.csv'
    compare_md = out_dir / 'S2_RPS_HIGH_COVERAGE_BRIDGE_COMPARE.md'
    dist_md = out_dir / 'S2_RPS_HIGH_COVERAGE_BRIDGE_DISTRIBUTION.md'
    analysis_md = out_dir / 'S2_RPS_HIGH_COVERAGE_BRIDGE_ANALYSIS.md'
