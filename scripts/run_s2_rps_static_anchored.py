...
def decide(rows: List[dict]) -> None:
    ...
    for row in rows:
        ...
        row['decision'] = 'iterate' if tb_ok and comp_ok and ghost_ok and metric_ok else 'abandon'
...
def write_compare(rows: List[dict], csv_path: Path, md_path: Path) -> None:
    ...
    lines = [
        ...
        '',
        '| variant | tum_acc_cm | tum_comp_r_5cm | bonn_acc_cm | bonn_comp_r_5cm | bonn_ghost_reduction_vs_tsdf | bonn_rear_points_sum | bonn_rear_true_background_sum | bonn_rear_ghost_sum | bonn_rear_hole_or_noise_sum | bonn_history_anchor_mean | bonn_surface_anchor_mean | bonn_surface_distance_mean | decision |',
        '|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|',
    ]
    for row in rows:
        lines.append(
            f"| {row['variant']} | {row['tum_acc_cm']:.4f} | {row['tum_comp_r_5cm']:.2f} | {row['bonn_acc_cm']:.4f} | {row['bonn_comp_r_5cm']:.2f} | {row['bonn_ghost_reduction_vs_tsdf']:.2f} | {row['bonn_rear_points_sum']:.0f} | {row['bonn_rear_true_background_sum']:.0f} | {row['bonn_rear_ghost_sum']:.0f} | {row['bonn_rear_hole_or_noise_sum']:.0f} | {row['bonn_history_anchor_mean']:.3f} | {row['bonn_surface_anchor_mean']:.3f} | {row['bonn_surface_distance_mean']:.4f} | {row['decision']} |"
        )
    md_path.write_text('\n'.join(lines) + '\n', encoding='utf-8')
...
def write_distribution(payloads: Dict[str, dict], rows: List[dict], path_md: Path) -> None:
    ...
    lines += ['', '重点检查：TB 是否突破 `>=8`，Comp-R 是否恢复到 `>=70.5%`，Noise 是否明显下降。']
    path_md.write_text('\n'.join(lines) + '\n', encoding='utf-8')
...
def write_analysis(rows: List[dict], path_md: Path) -> None:
    ...
    lines = [
        ...
        '',
        '## 1. 历史锚定是否提升了 TB',
        f"- 基组 `72`: rear=`{control['bonn_rear_points_sum']:.0f}`, TB=`{control['bonn_rear_true_background_sum']:.0f}`, Ghost=`{control['bonn_rear_ghost_sum']:.0f}`, Noise=`{control['bonn_rear_hole_or_noise_sum']:.0f}`",
        f"- 本轮最佳候选 `{best['variant']}`: rear=`{best['bonn_rear_points_sum']:.0f}`, TB=`{best['bonn_rear_true_background_sum']:.0f}`, Ghost=`{best['bonn_rear_ghost_sum']:.0f}`, Noise=`{best['bonn_rear_hole_or_noise_sum']:.0f}`",
        f"- 基组 history/surface=`{control['bonn_history_anchor_mean']:.3f}/{control['bonn_surface_anchor_mean']:.3f}`，候选=`{best['bonn_history_anchor_mean']:.3f}/{best['bonn_surface_anchor_mean']:.3f}`",
        f"- 基组 surface_distance=`{control['bonn_surface_distance_mean']:.4f}`，候选=`{best['bonn_surface_distance_mean']:.4f}`",
    ]
    if best['bonn_rear_true_background_sum'] > control['bonn_rear_true_background_sum']:
        lines.append('- 历史锚定提升了 TB。')
    else:
        lines.append('- 历史锚定没有提升 TB。')
    if best['bonn_rear_hole_or_noise_sum'] < control['bonn_rear_hole_or_noise_sum']:
        lines.append('- Noise 被清理。')
    else:
        lines.append('- Noise 没有被有效清理。')
    lines += [
        '',
        '## 2. Comp-R 是否恢复',
        f"- 基组 `72`: Bonn `Comp-R = {control['bonn_comp_r_5cm']:.2f}%`, `ghost_reduction_vs_tsdf = {control['bonn_ghost_reduction_vs_tsdf']:.2f}%`",
        f"- 候选 `{best['variant']}`: Bonn `Comp-R = {best['bonn_comp_r_5cm']:.2f}%`, `ghost_reduction_vs_tsdf = {best['bonn_ghost_reduction_vs_tsdf']:.2f}%`",
    ]
    if best['bonn_comp_r_5cm'] >= 70.5:
        lines.append('- Bonn Comp-R 恢复到目标线以上。')
    else:
        lines.append('- Bonn Comp-R 仍未恢复到 `70.5%`。')
    if best['bonn_rear_ghost_sum'] <= 25.0:
        lines.append('- Ghost 维持在可控上界内。')
    else:
        lines.append('- Ghost 仍超出 `25`。')
    lines += [
        '',
        '## 3. 诊断结论',
...
def main() -> None:
    ap = argparse.ArgumentParser(description='S2 static anchored rear runner.')
    ...
    variants = [
        dict(base_spec, name='72_local_geometric_conflict_resolution', output_name='72_local_geometric_conflict_resolution_static_anchor_control'),
        dict(
            base_spec,
            name='74_static_history_weight_boosting',
            bonn_rps_rear_selectivity_history_anchor_weight=0.65,
            bonn_rps_rear_selectivity_history_anchor_floor=0.32,
            bonn_rps_rear_selectivity_history_anchor_relief_weight=0.18,
            bonn_rps_rear_selectivity_topk=95,
        ),
        dict(
            base_spec,
            name='75_dynamic_shell_masking',
            bonn_rps_rear_selectivity_history_anchor_weight=0.35,
            bonn_rps_rear_selectivity_history_anchor_floor=0.24,
            bonn_rps_rear_selectivity_surface_anchor_weight=0.20,
            bonn_rps_rear_selectivity_surface_anchor_floor=0.20,
            bonn_rps_rear_selectivity_dynamic_shell_weight=0.55,
            bonn_rps_rear_selectivity_dynamic_shell_max=0.42,
            bonn_rps_rear_selectivity_dynamic_shell_gap_ref=0.035,
            bonn_rps_rear_selectivity_topk=85,
        ),
        dict(
            base_spec,
            name='76_surface_persistent_anchoring',
            bonn_rps_rear_selectivity_history_anchor_weight=0.30,
            bonn_rps_rear_selectivity_history_anchor_floor=0.22,
            bonn_rps_rear_selectivity_surface_anchor_weight=0.72,
            bonn_rps_rear_selectivity_surface_anchor_floor=0.42,
            bonn_rps_rear_selectivity_surface_anchor_risk_weight=0.32,
            bonn_rps_rear_selectivity_surface_distance_ref=0.035,
            bonn_rps_rear_selectivity_dynamic_shell_weight=0.20,
            bonn_rps_rear_selectivity_dynamic_shell_max=0.60,
    ...
    compare_csv = out_dir / 'S2_RPS_STATIC_ANCHORED_COMPARE.csv'
    compare_md = out_dir / 'S2_RPS_STATIC_ANCHORED_COMPARE.md'
    dist_md = out_dir / 'S2_RPS_STATIC_ANCHORED_DISTRIBUTION.md'
    analysis_md = out_dir / 'S2_RPS_STATIC_ANCHORED_ANALYSIS.md'
