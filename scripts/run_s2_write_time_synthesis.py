...
PY = str(Path('/home/zzy/anaconda3/envs/cgpm/bin/python'))
...
def dryrun_base_cmds(frames: int, stride: int, max_points: int) -> Tuple[List[str], List[str]]:
    dry = run([
        ...
        '--out_root', 'output/post_cleanup/_s2_cmd_seed',
        '--summary_csv', 'output/summary_tables/_s2_cmd_seed.csv',
...
def main() -> None:
    ...
    csv_path = PROJECT_ROOT / 'processes' / 's2' / 'S2_CONTROLLED_ABLATION_COMPARE.csv'
    md_path = PROJECT_ROOT / 'processes' / 's2' / 'S2_CONTROLLED_ABLATION_COMPARE.md'
    csv_path.parent.mkdir(parents=True, exist_ok=True)
