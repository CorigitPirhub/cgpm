...
COMPARE_PATH = PROJECT_ROOT / "processes" / "s2" / "S2_RPS_GEOMETRY_CHAIN_COUPLING_COMPARE.csv"
DONOR_ROOT = PROJECT_ROOT / "output" / "post_cleanup" / "s2_stage" / "99_manhattan_plane_completion"
TUM_REFERENCE_ROOT = PROJECT_ROOT / "output" / "post_cleanup" / "s2_stage" / "99_manhattan_plane_completion" / "tum_oracle" / "oracle"
REFACTOR_REPORT_PATH = PROJECT_ROOT / "processes" / "s2" / "S2_NATIVE_MAINLINE_REFACTOR_REPORT.md"
...
def write_refactor_report(rows: List[dict], path_md: Path) -> None:
    ...
    lines = [
        ...
        "## 3. 旧 runner 处置",
        "",
        "- `scripts/run_s2_rps_geometry_chain_coupling.py` 仅保留为诊断/对照脚本。",
